...
/// @title Canonical Leviathan factory
/// @notice Deploys Leviathan pools and manages ownership and control over pool protocol fees
contract LeviathanFactory is ILeviathanFactory {
    /// @inheritdoc ILeviathanFactory
    ...
    function createPool(
        ...
        pool = ILeviathanPoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        ILeviathanPool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return ILeviathanPool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        ILeviathanPool(pool).setLmPool(lmPool);
    }
}
