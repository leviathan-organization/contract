...
import '@leviathanswap/core/contracts/interfaces/callback/ILeviathanFlashCallback.sol';
import '@leviathanswap/core/contracts/libraries/LowGasSafeMath.sol';
...
/// @notice An example contract using the Leviathan flash function
contract PairFlash is ILeviathanFlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `leviathanFlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        ILeviathanPool pool = ILeviathanPool(PoolAddress.computeAddress(deployer, poolKey));
